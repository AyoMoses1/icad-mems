...
function checkMenuItemAccess(
  ...
  const resourceMap: Record<string, string[]> = {
    ...
    "admin": ["Admin", "Onboarding", "Application", "Applications", "Accreditation", "Accreditations", "Inspection", "Inspections"],
  ...
  const nameWords = item.name.split(/\s+/).map(w => w.replace(/[^a-zA-Z0-9]/g, ""));
  ...
  return permissions.some((perm) => {
    ...
    if (possibleResources.some(res => 
      res.toLowerCase() === permResource.toLowerCase()
    )) {
      return true;
    }
    ...
    if (nameWords.some(word => 
      word.toLowerCase() === permResource.toLowerCase()
    )) {
      return true;
    }
