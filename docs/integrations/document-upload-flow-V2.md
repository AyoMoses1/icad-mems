# How to Determine Documents Required for an Application V2
...
Administrators can now manage services and their requirements through dedicated endpoints:
...
**Note:** When creating service requirements with `Metric = "File/Document"`, the system validates that the `RequirementList` has `DocumentTypesId` configured.
